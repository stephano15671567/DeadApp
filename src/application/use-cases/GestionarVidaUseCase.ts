...
export class GestionarVidaUseCase {
  ...
  async darSenalDeVida(usuarioId: string, { frecuencia }: DarSenalVidaDto): Promise<ChequeoVida> {
    ...
    // Si no existe, crear uno nuevo con la frecuencia indicada (o por defecto)
    const frecuenciaUsada = frecuencia || ('3_MESES' as FrecuenciaChequeo);
    ...
    // Registrar la señal de vida en la entidad de dominio
    chequeo.darSenal();
